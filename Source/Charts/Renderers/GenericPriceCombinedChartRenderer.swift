...
open class GenericPriceCombinedChartRenderer: NSObject, DataRenderer {
    ...
    @objc open weak var chart: GenericPriceChartView?
    ...
    internal var _drawOrder: [GenericPriceChartView.PriceChartDrawOrder] = [.bar, .bubble, .line, .candle, .scatter]
    
    @objc public init(chart: GenericPriceChartView, animator: Animator, viewPortHandler: ViewPortHandler)
    {
        self.chart = chart
    ...
    internal func createRenderers() {
        ...
        for order in drawOrder
        {
            ...
            case .line:
                ...
                {
                    _renderers.append(GenericPriceLineChartRenderer(dataProvider: chart, animator: animator, viewPortHandler: viewPortHandler))
                }
    ...
    open func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        for renderer in _renderers
        {
            ...
            else if renderer is GenericPriceLineChartRenderer
            {
                data = (renderer as! GenericPriceLineChartRenderer).dataProvider?.lineData
            }
    ...
    open var drawOrder: [GenericPriceChartView.PriceChartDrawOrder]
    {
        get { _drawOrder }
        set
        {
